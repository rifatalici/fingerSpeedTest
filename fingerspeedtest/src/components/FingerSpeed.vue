<template>
  <div class="container jumbotron">
    <h1 class="display-2">Finger Speed Test</h1>
    <p class="lead">
      Klavye Hızını Test Etmek İçin Hemen Başla</p>
    <hr class="my-3">

    <div v-if="isFinish" class="alert alert-primary">
      <h3>Süre Bitti</h3>
      <p class="display-4">{{dks}} DKS </p>
      <span>Başarı İstatistiği : %{{truePercent}} </span><br>
    ...
    <div v-else>
      <div class="card">
        <div class="card-body">
          <span v-for="(word,key) in words.filter((data,index)=>index<20)" :key="key" v-bind:class="key!=0 || writingWordControl " class="words px-2">
            {{word}}
          </span>
        </div>
      </div>
        ...
        <div class="card-body bg-success">
          <div class="input-group input-group-lg">
            ...
            <div class="input-group-append" id="button-addon4">
              ...
              <button :disabled="isRunning" class="btn btn-danger" type="button" @click="getWords">Yenile</button>
            </div>
          </div>
        </div>
      </div>
